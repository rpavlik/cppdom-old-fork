...
#               there is only one suffix for object files ($(OBJECT_EXT)).
# -----------------------------------------------------------------------------
...
# generalized.
DEP_GEN_FLAG=	-M
DEPEND_FILES=	$(addprefix $(DEPDIR)/, $(BASIC_OBJS:.$(OBJECT_EXT)=.d))
...
_CC_SED_EXP=	'\''s|\($*\)\.$(OBJECT_EXT)[ :]*|$$(OBJDIR)/\1.$(OBJECT_EXT) $@: |g'\''
_MKDEP_SED_EXP=	'\''s|.*\($*\)\.$(OBJECT_EXT)[ :]*|$$(OBJDIR)/\1.$(OBJECT_EXT) $@: |g'\''
...
ifdef USE_MAKEDEPEND
   _C_DEPGEN=	$(SHELL) -ec 'makedepend -f- -o.$(OBJECT_EXT)		\
		   $(DEPENDFLAGS) -- $(DEPEND_EXTRAS) -- $< |		\
		   sed $(_MKDEP_SED_EXP) > $@ ; [ -s $@ ] || rm -f $@'
   _CXX_DEPGEN=	$(SHELL) -ec 'makedepend -f- -o.$(OBJECT_EXT)		\
		   $(DEPENDFLAGS) -- $(DEPEND_EXTRAS) -- $< |		\
		   sed $(_MKDEP_SED_EXP) > $@ ; [ -s $@ ] || rm -f $@'
else
